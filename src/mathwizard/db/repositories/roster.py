...
def _teacher_to_domain(row: TeacherSchema) -> Teacher:
    return Teacher(id=row.id, user_id=row.user_id)


def _student_to_domain(row: StudentSchema) -> Student:
...
class SqlAlchemyRosterRepository:
    ...
    def get_student_by_user_id(self, user_id: int) -> Student | None:
        ...
        row = self._session.scalars(statement).first()
        return None if row is None else _student_to_domain(row)
    ...
    def list_students_for_teacher(self, teacher_id: int) -> list[Student]:
        ...
        return [
            _student_to_domain(row) for row in self._session.scalars(statement).all()
        ]
