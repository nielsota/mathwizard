...
def test_seed_practice_questions_inserts_from_yaml(tmp_path: Path) -> None:
...
def test_seed_practice_questions_is_idempotent(tmp_path: Path) -> None:
    _write_practice_yaml(tmp_path, title="Machtsfuncties", difficulty=1)
    settings = Settings(
        db=DatabaseSettings(url="sqlite:///unused.db"),
        paths=PathSettings(repo_root=tmp_path),
...
def test_seed_practice_questions_updates_an_existing_question(tmp_path: Path) -> None:
    _write_practice_yaml(tmp_path, title="Machtsfuncties", difficulty=1)
    settings = Settings(
        db=DatabaseSettings(url="sqlite:///unused.db"),
        paths=PathSettings(repo_root=tmp_path),
